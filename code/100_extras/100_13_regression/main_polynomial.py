...
def train_and_predict(x, y, val_x, val_y, test):
	transformer = PolynomialFeatures(degree=2) # please try degree=3
	x_ = transformer.fit_transform(x)
	val_x_ = transformer.fit_transform(val_x)
	test_ = transformer.fit_transform(test)
	print(x_)

	model = LinearRegression()
	model.fit(x_, y)

	cod = model.score(val_x_, val_y)
